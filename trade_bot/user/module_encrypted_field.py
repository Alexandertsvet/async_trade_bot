...
class CustomEncryptedCharField(models.CharField):
    def __init__(self, *args, **kwargs):
        ...
        if not hasattr(settings, "FIELD_ENCRYPTION_KEY"):
            raise ValueError(
                "Не задан ключ шифрования FIELD_ENCRYPTION_KEY в settings.py"
            )
    ...
    def from_db_value(self, value, expression, connection, **kwargs):
        ...
        if value:
            try:
                return self.fernet.decrypt(value.encode("utf-8")).decode(
                    "utf-8"
                )
            except InvalidToken:
                logger.critical(
                    "Критическая ошибка дешифрования поля! Данные повреждены или изменены."
                )
                raise InvalidToken(
                    "Попытка чтения нешифрованных или поврежденных данных!"
                )
        return value
    ...
    def contribute_to_class(self, cls, name):
        ...
        if self.max_length is None:
            raise ValueError(
                "Длина строки для CustomEncryptedCharField должна быть задана"
            )
